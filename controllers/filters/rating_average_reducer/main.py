...
class RatingReducer:
    ...
    def update_average(self, title, average):
        ...
        #self.averages_per_title[title]['count'] += 1
    ...
    def calculate_new_average(self, title, rating):
        ...
        if new_average > 5:
            logging.debug(f"rating: {rating} old_count: {old_count} old_average {self.averages_per_title[title]['average']} old_total {old_total} new_total {new_total} new_average {new_average}")
        return new_average
    ...
    def _init_messaging(self):
        ...
        self.messaging.add_queues(self.OUTPUT_QUEUE, self.OUTPUT_QUEUE)
    ...
    def send_top_10_average_books(self):
        top_10_titles_and_averages = sorted(self.averages_per_title.items(), key=lambda x: x[1]["average"], reverse=True)[:10]
        data = list(
            map(
                lambda x: {"title": x[0]},
                top_10_titles_and_averages
            )
        )
...
def main():
    ...
    counter = RatingReducer(items_per_batch=filter_config.get("ITEMS_PER_BATCH"))
