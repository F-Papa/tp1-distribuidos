...
class DataAccess:
    ...
    class DataEntry:
        def __init__(
            ...
            if not isinstance(key, key_type):
                raise ValueError(f"Expected key of type {key_type} and got {type(key)}")
            if not isinstance(value, value_type):
                raise ValueError(
                    f"Expected value of type {value_type} and got {type(value)}"
                )
    ...
    def _write_oldest_to_disk(self):
        ...
        partition = self._partition_for_key(oldest_entry.key)
        ...
        entry = f"{serialized_key}{self.separator}{serialized_value}\n"
    ...
    def _parse_stored_value(self, stored_value: str) -> Any:
        json_parsed = json.loads(stored_value)
        return self.value_type(json_parsed)
    ...
    def _pop_from_disk(self, key: Any) -> Union[DataEntry, None]:
        ...
        with open(file_name, "r") as original_file, open(temp_file_name, "w") as temp:
            for line in original_file:
                separator_index = line.find(self.separator)
                ...
                if current_key == serialized_key:
                    serialized_value = line[
                        separator_index + len(self.separator) :
                    ]
                    ...
                    entry = self.DataEntry(
                        ...
                        self.value_type,
                        key,
                        self.value_type(json.loads(serialized_value)),
                    )
                    self.entries_in_files -= 1
    ...
    def _partition_for_key(self, key: Any) -> int:
