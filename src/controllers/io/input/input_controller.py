...
class InputController:
    ...
    def __init__(self, config: Configuration, messaging: Goutong):
        ...
        self._messaging.add_queues(DATE_FILTER_QUEUE, DECADES_COUNTER_QUEUE, CATEGORY_FILTER_QUEUE)
        ...
        self._books_sent = dict()
        self._books_received = dict()
    ...
    def _dispatch_books(self, _: Goutong, message: Message):
        ...
        while len(self._next_batch[conn_id]) >= self._items_per_batch:
        ...
        if end_of_books:
            self.__dispatch_books_aux(
                conn_id, self._next_batch[conn_id], True
            )
