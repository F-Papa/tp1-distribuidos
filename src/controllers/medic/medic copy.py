...
class Medic:
    ...
    def __init__(self, controllers_to_check: dict, config: Configuration):
        ...
        self.resolved_controllers = dict()
    ...
    def send_healthchecks(self):
        for _ in range(MSG_REDUNDANCY):
            for alias, address in self.controllers_to_check.items():
                ...
                crash_maybe()
                try:
                    if alias in self.resolved_controllers:
                        address = self.resolved_controllers[alias]

                    self.healthcheck_sock.sendto(
                        message.encode(), (address, CONTROL_PORT)
                    )
                except socket.gaierror:
                    logging.error(f"Could not resolve address: {address}")
    ...
    def send_healthcheck_response(self, address, controller_id, seq_num):
        ...
        try:
            if controller_id in self.resolved_controllers:
                address = self.resolved_controllers[controller_id]
    ...
    def elect_leader(self):
        ...
        for i in range(self.medic_number, self.number_of_medics):
            ...
            medic_id = f"medic{i+1}"

            if medic_id in self.resolved_controllers:
                address = self.resolved_controllers[medic_id]
            else:
                address = medic_id
            try:
                for _ in range(MSG_REDUNDANCY):
            ...
            for _ in range(self.number_of_medics):
                ...
                for i in range(0, self.medic_number):
                    ...
                    try:
                        for _ in range(MSG_REDUNDANCY):
                            self.control_sock.sendto(
                                coordinator_msg.encode(), (address, CONTROL_PORT)
                            )
                    except socket.gaierror:
                        continue
                    except Exception as e:
    ...
    def control_listen_thread(self):
        ...
        while not self._shutting_down:
            ...
            self.resolved_controllers.update({controller_id: address[0]})
            ...
            if response_code == Message.OK.value:
                ...
                if controller_id in self.resolved_controllers:
                    dest_address = self.resolved_controllers[controller_id]
                else:
                    dest_address = controller_id
                try:
                    for _ in range(MSG_REDUNDANCY):
                        crash_maybe()
                        self.control_sock.sendto(
                            ok_msg.encode(), (dest_address, CONTROL_PORT)
                        )
                except socket.gaierror:
                    pass
                except Exception as e:
    ...
    def listen_healthcheck_responses(self) -> dict:
        ...
        timed_out = False
        while not self._shutting_down and not timed_out:
            new_timeout = max(
            ...
            try:
                while len(data) == 0 or data[-1] != terminator_bytes:
                    ...
                    data += recieved
                    if data and data[-1] != terminator_bytes:
                        logging.error(f"Received incomplete message: {data.decode()}")
            ...
            if response_code == Message.IM_ALIVE.value:
                self.controllers_to_check[controller_id] = address[0]
