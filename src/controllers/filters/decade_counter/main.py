...
class DecadeCounter:
    ...
    def __init__(
        ...
        self._controller_id = f"{INPUT_QUEUE_PREFIX}{self._filter_number}"
    ...
    def _normalize_counts(self, saved_counts):
        normalized_counts = {
            conn_id: {
                author: list(decades) for author, decades in authors_and_decades.items()
            }
    ...
    def _get_10_decade_authors(self, conn_id: str):
        ...
        ten_decade_authors = [
            author
            for author in decades_by_author
...
def main():
    ...
    controller_id = (
        f"{DecadeCounter.CONTROLLER_NAME}_{filter_config.get('FILTER_NUMBER')}"
    )
    ...
    output_queues = {
        (2,): {"name": OUTPUT_QUEUE_PREFIX, "is_prefix": True},
    }
